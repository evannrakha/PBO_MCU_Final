...
public class PasienController {
    ...
    public String dashboardPasien(@RequestParam("idPasien") int idPasien, 
    @RequestParam(value = "jenisPemeriksaan", required = false) String jenisPemeriksaan, 
    Model model){
        ...
        List<PaketMCU> paketMCUList;

        if (jenisPemeriksaan != null && !jenisPemeriksaan.isEmpty()) {
            paketMCUList = paketMCUService.cariPaketByjenisPemeriksaan(jenisPemeriksaan);
        }else{
            paketMCUList = paketMCUService.semuaPaket();
        }

        model.addAttribute("pasien", pasien);
        model.addAttribute("listPaket", paketMCUList);
        model.addAttribute("jenisPemeriksaan", jenisPemeriksaan);
